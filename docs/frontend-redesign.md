...
- Make the README useful to Chinese readers before they reach the install section, including the quickstart, the detailed usage manual, and active Chinese example skills.
...
- The README must clearly explain whether to use the project in Codex as a skill or open it in VS Code for editing and validation.
- Native-language rows must link to active skill files and active vocabulary references, not migrated legacy files.
- Chinese vocabulary and Chinese example routes must be visible from the README.
