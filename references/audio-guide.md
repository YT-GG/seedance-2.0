...
Field-observed from 2026 community testing, with Mandarin consistently reported as the strongest non-post workflow; test per surface and never promise results. Use these as a reasoning model, not guarantees.
...
- Language strength is uneven. In this Chinese-focused package, treat Mandarin as the preferred spoken-language path, keep lines short, and use voice references or post-dub when reliability matters.

## Dialogue capacity (field-observed)

No official Mandarin word limit is published; the numbers below are field-observed ranges, not guarantees - test per surface.
...
Past the reliable-sync budget, use the voice-reference lip-sync path below or plan a post-dub.
...
- Mandarin dialogue: keep lines short and split long paragraphs into one-breath speaker turns. For a fully voiced piece, plan a post-dub when reliability matters.
...
On surfaces that accept a spoken-voice audio reference, field reports describe a stronger role than tempo or mood: attaching an actual voice clip can make the model lip-sync to that audio instead of synthesizing speech itself - effectively a lip-sync compiler. For Mandarin dialogue, record or commission the line, attach it, and let the model only move the mouth when reliability matters. Use only your own recorded, licensed, or rights-cleared voice; treat a real or recognizable person's voice as authorization-sensitive and route it through `[skill:seedance-copyright]` when rights are unclear. Verify the active surface actually exposes a voice audio reference before relying on this.
